...
class UserRepository extends com.github.crown.util.CrownDataSource {
    ...
    void deleteUserRoleTableByUserName(userName) {
        def deleteSysUserRoleSql = "delete from sys_user_role where uid in (select user_id from sys_user where login_name=?)"
        crownDB.execute(deleteSysUserRoleSql, [userName])
    }

    def insertSysUserRoleTable(uid, roleId) {
        def insertSysUserRoleSql = "insert into sys_user_role(user_id,role_id) values(?,?)"
    ...
    def getUserRole(loginName) {
        def getUserRoleSql="select * from sys_user_role where user_id in (select user_id from sys_user where login_name =?)"
